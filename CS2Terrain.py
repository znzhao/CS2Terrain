...
with expand1:
    ...
    ks2 = st.selectbox('Reverse Kernel Size' if not CHN else "复原平滑卷积核", index = 0, options=[4, 8, 16, 32, 64, 128])
...
if st.session_state.input_height is not None:
    ...
    input_map = go.Figure(data=go.Contour(z=st.session_state.input_height*4000, colorscale=color_continuous_scale)) if not threeD else go.Figure(data=[go.Surface(z=st.session_state.input_height*4000, colorscale=color_continuous_scale)])
...
if st.session_state.output_height is not None:
    output_map = go.Figure(data=go.Contour(z=st.session_state.output_height*4000, colorscale=color_continuous_scale)) if not threeD else go.Figure(data=[go.Surface(z=st.session_state.output_height, colorscale=color_continuous_scale)])
    ...
    terrain = cv2.resize(st.session_state.output_height, (4096, 4096), interpolation=cv2.INTER_AREA)*65536
    terrain = cv2.GaussianBlur(terrain, revise_kernel, 0)
    terrain = terrain.astype(np.uint16)
    is_success, buffer = cv2.imencode(".png", terrain)
    io_buf = io.BytesIO(buffer)
    with io_buf as file:
        st.download_button(label="Download" if not CHN else "下载地形图", data = file, file_name='Output.png', mime="image/png",type = 'primary')
